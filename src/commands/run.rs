...
pub fn run(version: String, args: Vec<String>) -> Result<()> {
    let base_dirs = BaseDirs::new().context("Could not determine home directory")?;
    let install_dir = base_dirs.data_local_dir().join("blup").join("versions").join(&version);

    if !install_dir.exists() {
        bail!("Blender {} is not installed. Run `blup install {}` first.", version, version);
    }
